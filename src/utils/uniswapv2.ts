...
import { type WalletClient, getCreate2Address, keccak256, encodePacked, type PrivateKeyAccount } from "viem";
import { ROUTER_V2_ABI } from "../constants/abi/ABIs";
import { DEADLINE } from "../config/config";
import { UNISWAP_V2_ROUTER_ADDRESSES, WETH_ADDRESSES, UNISWAP_V2_FACTORY_ADDRESSES, INIT_CODE_HASHES } from "../constants/addresses";
...
export const pairFor = (tokenA: `0x${string}`, tokenB: `0x${string}`, chainId: number): `0x${string}` => {
    const tokenOrder: readonly [`0x${string}`, `0x${string}`] = tokenA < tokenB ? [tokenA, tokenB] : [tokenB, tokenA];
 ...
 */
export const buy = async (walletClient: WalletClient, account: PrivateKeyAccount, buyAmount: bigint, token: `0x${string}`, chainId: number) => {
...
export const sell = async (walletClient: WalletClient, account: PrivateKeyAccount, sellAmount: bigint, token: `0x${string}`, chainId: number) => {
