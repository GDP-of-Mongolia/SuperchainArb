...
import { type WatchEventReturnType, type PublicClient, parseAbi, type PrivateKeyAccount } from 'viem';
import { watchSyncEvents } from '../utils/monitor';
import { BRIDGE_SWAP_CONTRACT } from '../config/config';
...
export class Executor {
     ...
     */
    constructor(tokensAndInstances: Map<`0x${string}`, V2Instance[]>, arbAccount: PrivateKeyAccount) {
    ...
    executeArb = async (execution: V2ArbExecution) => {
        ...
        const partialABI = parseAbi(["function swapAndBridge(address owner, address token, address recipient, uint256 amount, uint256 originChainId, uint256 destinationChainId, address originRouterV2, address destRouterV2, address destContract)"])
